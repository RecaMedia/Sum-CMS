...
function addedUser($From,$Fname,$Email,$Pass,$Confirm,$Domain){
	if($Confirm==0){
		$EncodeEmail = urlencode($Email);
		$ActiveMsg = $GLBLang['msg1'];
	}else if($Confirm==1){
		$ActiveMsg = $GLBLang['msg2'];
	}
	
	$Message = $GLBLang['msg3'];
	
	if(mail($Email,$GLBLang['msg4'].' '.$Domain,$Message,'From: '.$From,'-f '.$From)){
		return true;
	}else{
		return false;
	}
